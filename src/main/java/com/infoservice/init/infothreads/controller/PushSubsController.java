...
public class PushSubsController {
    ...
    @CrossOrigin
    public Object addPushSub(@Valid @RequestBody PushSubscriptions pushSub) {
        JSONObject jsonObject = new JSONObject();


        pushSub.set_id(ObjectId.get());
        if (pushSubsRepo.findBykeys_auth(pushSub.keys.auth).isEmpty()) {
            pushSubsRepo.save(pushSub);
            return jsonObject.put("message", "new sub added");

        }

         return jsonObject.put("message", "already subbed");
    }
